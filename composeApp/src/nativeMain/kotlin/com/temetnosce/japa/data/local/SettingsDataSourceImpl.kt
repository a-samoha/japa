...
class SettingsDataSourceImpl() : SettingsDataSource {
    ...
    override suspend fun load(): Settings {
        ...
        return Settings(
            language = defaults.strForKey(language),
    ...
    override suspend fun edit(settings: Settings): Result<Unit> =
        kotlin.runCatching {
            val defaults = NSUserDefaults.standardUserDefaults()
            defaults.setBool(settings.developerMode, forKey = language)
            defaults.setBool(
                settings.designUseSystemColorPalette, forKey = designUseSystemColorPalette
            )
